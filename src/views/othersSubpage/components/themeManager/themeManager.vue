<template>
    <div class="container" @click="clickButton">
        <div class="sky" :style="{animationName:to_light?'sky-to-light':'sky-to-dark'}">
            <div class="cloud" :style="{animationName:to_light?'cloud-to-light':'cloud-to-dark'}"></div>
            <div v-if="is_light" class="light" :style="{animationName:to_light?'ele-in':'ele-out'}"></div>
            <div v-if="is_dark" class="dark" :style="{animationName:to_light?'ele-out':'ele-in'}"></div>
        </div>
    </div>
</template>
...
.light{
    background: linear-gradient(rgb(246, 250, 14) 0%, hsl(42, 100%, 70%) 45%, rgb(205, 163, 96) 80%);
    -webkit-mask-image: url("./img/light.png");
}
.dark{
    background: linear-gradient(rgb(253, 255, 205) 0%, hsl(61, 39%, 79%) 45%, rgb(194, 189, 149) 80%);
